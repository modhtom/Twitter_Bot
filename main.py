...
def handle_daily_tweets():
    ...
    while True:
        try:
            ...
            if is_friday(now):
                fri.fri()
                Thread(target=fri.friday_sala_dua).start()
            ...
            if (
                not tweetTwicePerDay
                or ((now - tweetTwicePerDay).total_seconds() >= 12 * 60 * 60)
                and double_tweet_count < 2
            ):
                double_tweet_tasks = [
                    other.tweet_blogs,
...
if __name__ == "__main__":
    try:
        # TODO: UNCOMMENT IF HOSTED TO MAKE THE BOT STAY A LIVE IF HOSTED
        # live.keep_alive()
        while True:
